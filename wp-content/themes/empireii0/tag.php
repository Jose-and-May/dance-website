		...
		//REDIRECT IF HEADER IS NOT LOADED, E.G. CALLING PAGE DIRECTLY
			...
			if($frontpage == get_the_ID() && empty($_GET['ajaxload'])){
				...
				print '<div class="hidden" id="page-title" data-id="'.get_the_ID().'">';
					...
					print __('Tag archives for','empire').' '.single_tag_title( '', false );
					$tit = __('Tag archives for','empire').' '.single_tag_title( '', false );
...
//AJAX IS OFF
		...
		get_header(); 
				...
				';
					...
					//title
					print '<h1 class="tp-title">'.__('Tag archives for','empire').' '.single_tag_title( '', false ).'</h1>				
					<div class="vspace2"></div>';
