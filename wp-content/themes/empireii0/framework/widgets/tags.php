...
class tp_widget_tags_cats extends WP_Widget {
	...
	function tp_widget_tags_cats() {
		$widget_ops = array('classname' => 'tp_widget_tags_cats', 'description' => __('Display Categories with tag graphic','empire') );
		$this->WP_Widget('tp_widget_tags_cats', '* Categories', $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		if(!empty($instance['tp_widget_tags_cats_title'])){ print $before_title . $instance['tp_widget_tags_cats_title'] . $after_title; }
			...
			if(empty($instance['tp_widget_tags_cats_list'])){ 
				...
				print '<ul class="widget-tp-tags">';
				foreach($cats as $cat){
					$cat_link = get_category_link($cat->term_id);
					print '<li><a href="'.$cat_link.'">'.$cat->name.'</a></li>';
				}
				print '</ul>';
			}else{
				print '<ul class="widget-tp-tags">';
				foreach($instance['tp_widget_tags_cats_list'] as $cat){					
					print '<li><a href="'.get_category_link($cat).'">'.get_cat_name($cat).'</a></li>';
				}
				print '</ul>';
			}
	...
	function form($instance) {
		...
		<p><label><b>'.__('Select categories to display','empire').'</b></label></p>
